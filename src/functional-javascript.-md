...
- task
  ...
  - 实现一个函数，该函数将一个函数作为第一个参数，一个 num 作为第二个参数，然后执行传入的函数 num 次。
...
- task
  ...
  - GetShortMessages 占有一系列具有“.message”属性的对象，并返回长度为<50 个字符的消息数组。
...
- task
  ...
  - 写一个函数 duckcount，返回传递给它的参数的数量，它具有直接定义的属性“Quack”。 不匹配从原型继承的值。
...
- task
  ...
  - 使用局部应用程序创建一个将第一个参数修复到 console.log 的函数。 即，实现将命名空间字符串添加到其输出的日志记录函数。
...
- Task
  ...
  - 修改位板中提供的递归重复功能，使得它不会阻止事件循环（即计时器和 IO 处理程序可以射击）。 这必然需要重复才能异步。

  - 超时在 100 毫秒后排队射击，这将打印测试结果并退出该过程。 重复应该释放对事件循环的控制，以允许在所有操作完成之前中断时断。
...
- task
  ...
  - 用户的顺序应匹配提供的用户 ID 的顺序。 因为这个函数是异步的，我们不关心其返回值
- Arguments
