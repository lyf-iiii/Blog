...
- 是一门全面的、可拓展性、能在服务端运行的js机制
## nodejs 的 非阻塞 I/O 排除掉一些不必要的等待
- I/O即 input和output，一个系统的输入和输出
- 阻塞I/O和非阻塞I/O的区别就是在于`系统接收输入再到输出期间，能不能接收其他输入`
- 排队打饭 vs 餐厅点菜
- 排队打饭是阻塞I/O 餐厅点菜是非阻塞I/O
- 饭堂阿姨只能一份一份的打饭 阻塞I/O 而 服务生点完菜之后可以服务其他客人 非阻塞I/O
## node事件循环 event loop
...
## node js 采用commonjs
- 没有exports时 commonjs require返回的是一个空对象
- commonjs exports的对象和require的返回值 是同一个对象会发生修改
## nodejs 内置模块
nodeJs的源码是如何获取系统信息的 通过v8引擎获取内置的一些c++模块然后返回的
### 什么是事件循环
